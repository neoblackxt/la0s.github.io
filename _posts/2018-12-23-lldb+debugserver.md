...
***
**2021.8更新：** iOS动态调试强烈推荐使用[voltron](https://github.com/snare/voltron)这个工具，可以打造媲美IDA的调试界面。我一般开command、registers、disasm、memory这四个窗口，调试过程中对于寄存器的变化和内存写入的监控操作都会以高亮显示（类似于Windows平台的Ollydbg），lldb+voltron应该是我在iOS平台下体验过最好的调试方式了。
